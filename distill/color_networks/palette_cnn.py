...
class PaletteCNN(nn.Module):
    def __init__(self, num_colors, layer_ids=[3, 7, 11], in_channel=3, im_size=(32, 32), soften=1, color_norm=1, color_jitter=0, temp=1.0, topk=4, agg='mean'):
        ...
        # self.palette = torch.nn.Parameter(torch.randn(3, num_colors))
        
        self.palette_generator = nn.Sequential(
            nn.Linear(im_size[0]*im_size[1], 100),
            nn.Linear(100, 1)
        )
    ...
    def forward(self, img, activation_maps=None, training=True):
        ...
        # color_palette = self.palette.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).repeat(B, 1, 1, H, W)
        color_weighted = (img.unsqueeze(2) * m.unsqueeze(1)).flatten(start_dim=-2)
        color_palette = self.palette_generator(color_weighted).unsqueeze(-1).repeat(1, 1, 1, H, W)
